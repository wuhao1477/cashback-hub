...
export function formatDateRange(start?: string, end?: string) {
  ...
  const startText = start ? dayjs(start).format('MM/DD HH:mm') : '即刻生效';
...
export function inferStatus(start?: string, end?: string): ActivityStatus {
  ...
  if (startTime && now.isBefore(startTime)) return 'upcoming';
  if (endTime && now.isAfter(endTime)) return 'offline';
  if (startTime || endTime) return 'online';
  return 'unknown';
}
